...
class LSUNEncoder(nn.Module):
    def __init__(self, image_size, latent_size, num_chanel, hidden_chanels=64):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(self.hidden_chanels * 4, self.hidden_chanels * 8, [2, 3], 2, 1, bias=False),
            nn.BatchNorm2d(self.hidden_chanels * 8),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*8) x 4 x 5
            nn.Conv2d(self.hidden_chanels * 8, self.latent_size, [4, 5], 1, 0, bias=False),
        )

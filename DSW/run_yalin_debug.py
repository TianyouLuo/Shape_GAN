...
image_size = 100 * 150
num_chanel = 1
outdir = '/pine/scr/t/i/tianyou/Yalin_GAN/DSW/result/'
niter = 10
lam = 10
batch_size = 64
epochs = 100
...
if model_type == "SWD" or model_type == "JSWD":
    model_dir = os.path.join(outdir, model_type + "_n" + str(num_projection))
    ...
    model_dir = os.path.join(
        outdir, model_type + "_iter" + str(niter) + "_n" + str(num_projection) + "_lam" + str(lam)
    )
...
if(dataset=='Yalin'):
    ...
    train_data = np.zeros((len(files),100,150), dtype=np.float32)
...
if model_type == "DSWD" or model_type == "DGSWD":
    transform_net = TransformNet(100 * 150).to(device)
    op_trannet = optim.Adam(transform_net.parameters(), lr=lr, betas=(0.5, 0.999))
...
fixednoise = torch.randn((128, latent_size)).to(device)
ite = 0
wd_list = []
swd_list = []
for epoch in range(epochs):
    ...
    for batch_idx, data in tqdm(enumerate(train_loader, start=0)):
        if model_type == "SWD":
            loss = model.compute_loss_SWD(data, torch.randn, num_projection, p=p)
